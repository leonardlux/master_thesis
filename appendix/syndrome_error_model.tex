...
\caption{
    ...
    Here the channel are marked at their positions.
...
This chapter discuses how the circuit-level error model for a STEC circuit, can be mapped to a \emph{syndrome error model} (SEM).
The syndrome error model, describes the errors observed by the syndrome measurement\footnote{
    ...
    We refer to it as syndrome noise model to highlight, that it only contains the modeling for the syndrome,
    the effect on the data qubit is completely neglected, in its description.
...
It is calculated by propagating/commuting the error channels, respectively their operators, trough the circuit and thus determining their influence on the syndrome. 
...
As the STEC circuit and circuit-level error model is transversal\footnote{
...
it can be holds for all possible underlying encodings compatible with STEC. 

Applying the circuit-level noise model to STEC results in the noise model shown in figure \ref{circ:start_circ_lvl_noise}.
As indicated in the circuit figure we use the noise channels introduced in chapter \ref{ssec:error_models}.
This included the bit- and phase-flip noise channel (equations \ref{eq:bit_flip_channel}, \ref{eq:phase_flip_channel}), 
...
Figure \ref{circ_dia:propagated_noise} illustrate how the syndrome noise model will look like. 
The `N' measurement indicates that all operators from the noise model acting on the data qubit are neglected.
...
One can observe that the $X$-errors has no effects on the $X$-measurements performed on the ancilla  $\ket{0}_L$.
$X$-errors on the $\ket{0}_L$ state can therefore be neglected for the purpose of the SEM, resulting in $X~I$ and $Y~Z$. 
Applying the same logic for the $\ket{+}_L$, results in $Z~I$ and $Y~X$. 
...
\end{quantikz}
\caption{
    TOOOODOOOOO extend caption of this circuit!!!
    Circuit diagram with final possible error models shown.
    }
...
The calculations of the SEM follows the following steps:
\begin{enumerate}
    \item Starting point is the circuit-level noise model, consisting of independent Pauli channels shown in \ref{circ:start_circ_lvl_noise}. 
    ...
    \begin{itemize}
        ...
        \item The CNOT gate, which represents the non trivial part of the propagation, are discussed in section \ref{ssec:cnot_gate}.
    ...
    \begin{itemize}
        ...
        \item The resulting SEM contains correlations between $X$- and $Z$-errors and is shown in section \ref{sssec:sem_results_correlated_sem}.
    \end{itemize}
...
The results of this process is discussed in section \ref{ssec:sem_results}.
...
We this process by shortly showing that all Pauli Channels commute. 
To show that all Pauli channel commute we define two arbitrary Pauli channels $N_1$ and $N_2$
\begin{align}
...
Because all Pauli channel they can be propagated independently of one another to the end of the circuit. 
...
The SEM is used to model the noises influence on the syndrome.
Therefore all the noises processes, which have no impact on the syndrome can be neglected.
This we refer to this as \emph{syndrome equivalence}, and it is formalized in this section.

This chapter discuses the equivalence of operators after they are propagated thorough the CNOT circuit.
...
For operator on the data qubit we have to take the following measurement into account, depending if they commute or anticommute, 
...
A $Z^s$ error will be corrected by a $Z$ operator at the same qubit index on the encoded block\footnote{
...
In this section the propagation of each operator of the channels is performed in the following three steps: 
\begin{enumerate}
...
\begin{align*}
    ...
    &\overset{\tilde{s}}{\rightarrow} Z^s  \\
\end{align*} 
applying those mapping to the channel equation results in  
\begin{align*}
...
\end{equation}
Note the that function is symmetric the with respect to its argument, $f_{2}(p_1,p_2)=f_{2}(p_2,p_1)$, 
...
The resulting channels is a bit-flip channel with the associated probability  $p_x = \frac{2}{3} p_D$
...
can be combined using the equations describing the composition of channels, shown in \ref{ssec:sem_composition_of_channels}.
...
which probabilities are combined using equation \ref{eq:composition_flip_channel} for the flip channels and equation \ref{eq:composition_depo_depo_prob_simple} for the depolarizing channels.
...
The efficient ML decoder used in this thesis is only designed independently decode bit- and phase-flip errors \cite{efficient_decoding_algorithm}.
...
Implementing this adaption is trivial as it trivially requires to map one kind of error operator to identity.
For example the bit-flip syndrome error model 
\begin{align*}
    X^s, Y^s &\rightarrow X^{s_x}\\
    Z^s, I^s &\rightarrow i^{s_x}.
\end{align*}
As such the depolarizing noise channel get mapped onto a flip error, as discussed in section \ref{para:depo_to_flip}, 
...
In this thesis we use those SEM, one for each half-cycle, as the underlying error model for efficient ML decoder. 
As described in section \ref{ssec:error_models}, the simulations use an error model in which all channels have the same defining probability $p_{phy}$.
% uniform p channel! ?!
...
As such the SEM for the different order can be obtained from the above results be performing this basis change. 
...
For repeated syndrome extraction cycles the modeling of the incoming error would likely needs adaption, as the incoming error changes its form as seen in section \ref{ssec:results_circ_multiple_cycles}. 
...
It will be shown that such symmetric SEM exists, but they are only appear in the regime of high noise rates, which is outside of the are of interest of QEC. 
...
The motivation behind finding a set of probabilities resulting a symmetric SEM, 
...
and the discussion around equation \ref{eq:sem_depo_depo_not_negative} has shown that the provability of a composite channel is only equal to zero, if all its components are equal to zero.
This case is not relevant for the analysis as circuit level noise, as we need to need to assume multiple components as faultless. 

The second case $p_{D'}^f = 3/4 $ is also not of interested as the resulting SEM can not contain any information about the occurred error. 
...
\end{align*}
In this case we effective cut the wire of one of the ancilla qubit, fully randomizing one half of the syndrome.
...
Below is shown which individual channel probabilities lead to the discussed cases\footnote{We neglected the $p_{D'}^f = 3/4 $ case, as it is fully depolarizing.} 
\begin{align}
...
The SEM take the form of a depolarizing channel if 
\begin{equation}
...
This also directly requires to assume that components of the circuit can be faultless, which is going against the concept of a circuit-level noise model.
...
The correlation between the SEM and the errors on the data qubit can be analyzed by performing computation to obtain the SEM without neglecting the operators on the data qubit. 
As the resulting model becomes more complicated, as we are not any more limited to only depolarizing, bit- and phase-flip channels, we performed such computation only up to first order in the probabilities.
...
we can analysis the probability of $X$- and $Z$-discrepancy between the the error on the data qubit and the syndrome. 
...
\begin{equation}
    ...
    1-P_{X,\mathrm{res}}
\end{equation}
which trivially states that extracting both type of errors accurately is less likely than correct any type of error individually.
Therefore it is expectable that redesigning the quantum memory experiment, such that it is is only successful if both types of error are corrected correctly,
lead to a diminished logical error rate.  
Given that the logic error rate curves will change, the threshold value will most likely change as well.
