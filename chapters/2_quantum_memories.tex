...
Before we discuss how to correct error, we first need to discuss what types of errors can occur and how we can model them \cite{terhal_qec_formalism}, \cite{gottesmann_thesis} and \cite{intro_to_qec}.
...
The goal of this chapter is to introduce the structure and formalism behind \emph{stabilizer codes} \cite{gottesmann_book,gottesmann_thesis,terhal_qec_formalism}.
...
Based on the eigenvalues we can assign the state a $m$-bit string, called the \emph{syndrome}, $\sigma(\ket{\Psi}) \in \mathbb{Z}_2$, following 
\begin{equation}
...
Motivated by the above observed importance of commutation relations between operators and stabilizers regarding the syndrome, we look at the \emph{centralizer}\footnote{Also known as commutant.} $\mathcal{C}(\mathcal{S})$ of the stabilizer group $\mathcal{S}$
\begin{equation}
...
The \emph{distance} $d$ of a QEC code is the minium weight of a non-identity logical operator/undetectable error.
\begin{equation}
...
\begin{figure}[ht]
    ...
    \caption{
        This figure shows a visual representation of equation \ref{eq:pauli_rewriting_stabilizer_formalism} and its adopted from \cite{gottesmann_book}.
        ...
        The first black block show the centralizer $\mathcal{C}(\mathcal{S})$, including the stabilizer group $\mathcal{S}$ is marked by the green colored area.
        ...
        Each of the cosets $\hat{\mathcal{S}}$ in the centralizer realizes a different logical operator. 
        ...
        Each following black box is the showing a cosets of the centralizer $E_\sigma\mathcal{C}(\mathcal{S})$, and this image should in principle extend up until the $2^m$ cosets of the centralizer are shown.
        }
...
The specific QEC code from the class of the stabilizer codes we are using is the \emph{surface code}\footnote{
    We use the unrotated version because an efficient ML decoder exist for this QEC code \cite{efficient_decoding_algorithm}.
...
As described in section \ref{sec:stabilizer_formalism}, a stabilizer code can be specified by fixing the stabilizer group, or the generator of this group.
For the surface code this set can be split up into site and plaquette stabilizers, 
where the site stabilizer are only using $X$-type Pauli operators and plaquette stabilizers are only using $Z$-type Pauli operators.
...
Site stabilizers are placed on the sites of the lattice and act (nontrivial) on each physical qubit sitting on edges incident to this site with a $X$-type Pauli operator.
...
The complete resulting stabilizer setup is the visualized in the same figure as d).
...
\begin{figure}[ht]
    ...
    \caption{
        ...
        The first, figure a), shows the labeling and position of the physical qubits at each edge of the lattice.
        The labeling is identical with the one used in the implementation, see section \ref{chapter:git_repo} for the source code. 
        Figures b),c) and d) are displaying the generators of the stabilizer group and on which physical qubits each acts, 
        indicated by the colored area they show.
        Each qubit connected to the area is acted upon with the associated Pauli operator.
        Figure e) and f) show one possible realization of the logical operators $\bar{X}$ and $\bar{Z}$ respectively. 
        }
...
Connecting the boundaries of the surface code is therefore analogous to the noncontractable loop of the toric code \cite{topological_quantum_memory}.
...
This symmetry is exploited in the adaptation\footnote{
    Instead of reimplementing a new decoder, we can just change rotate the syndrome by $\pi/2$, to switch between bit- and phase-flip errors.
...
The circuit-level noise model breaks this symmetry due asymmetry between $X$ and $Z$ introduced by the syndrome-extraction circuit.
