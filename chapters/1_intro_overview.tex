...
There are many different QEC codes and configurations, each with its own advantages and disadvantages.
...
we can afford to accept the tradeoff of using more physical qubits in exchange for a constant-time QEC cycle.
A major part of each QEC cycle is the syndrome extraction circuit, 
which extract the information needed for decoding and correction, the syndrome, from the encoded state without compromising the information contained within it. 
...
To realize STEC, we need a quantum computing platform that either supports nearest-neighbor interactions beyond a single plane, 
or supports CNOT gates over long distances, such as a trapped-ion quantum computer \cite{friederike_steane}. 
As a resource, STEC uses logically encoded ancilla qubits, which enable the constant-time syndrome extraction, 
since encoding the ancilla allows for a parallel readout of all stabilizers through the use of transversal logical CNOT gates.
...
In other words, the threshold marks the point at which scaling up the QEC code leads to a suppressed logical error rate.
...
We characterize the performance of Steane-type error correction by determining the thresholds of a QEC code utilizing STEC.
...
We find a threshold results of roughly $3\%$ for a single QEC cycle which is a factor of three greater than the conventional syndrome extraction circuit.
further results and analysis can be found in the result chapter, chapter \ref{chapter:results}. 
...
\begin{enumerate}
    ...
    The general task of the decoder, along with the underlying principles behind \emph{minimum weight perfect matching} (MWPM) and \emph{maximum likelihood} (ML) decoding, 
    will be introduced as part of this chapter in section \ref{sec:decoding_problem_general}, and the specific decoders in subsections \ref{ssec:general_MWMP_decoder} and \ref{ssec:general_ML_decoder} respectively.
    ...
    Together, these sections provide the theoretical basis for the remainder of the thesis.
    ...
    focusing on the adaptation needed for circuit-level noise, which results in the required repetition of the syndrome extraction circuit for a single QEC cycle and in adaptations to the decoder, 
    ...
    % threshold theorem
    \item Chapter \ref{chapter:qec_thresholds} introduces the threshold and explains the process used to determine it based the simulations of the circuit. 
    ...
    \item We conclude this thesis with chapter \ref{chapter:conclusion}, in which we summarize the important findings of the previous chapters and contextualize those with the underlying assumptions of our approach.
