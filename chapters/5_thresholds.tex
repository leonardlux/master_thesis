\chapter{QEC Thresholds}\label{ssec:threshold_theorem}
...
given that the error rates of the noisy components $p_{phy}$ is below a certain \emph{threshold} $p_{th}$ if we use a fault-tolerant QEC code \cite{gottesmann_book,topological_quantum_memory}.
...
and vary the choice of error model (section \ref{chapter:error_models}), syndrome extraction circuit (section \ref{chapter:syndrome_extraction}), decoder (section \ref{sec:decoding_problem_general}) and the logical observable.
...
In the last step we use the chosen decoder, to decode the syndrome returning a prediction for the occurred error.
...
We can not detect if a logical $X$-type ($Z$-type) error occurred if we use a $\ket{+}_L$ ($\ket{0}_L$) state, given that those error operators are eigenstate of those logical operators.
...
An example of how the logical error curves $p_{log,d}(p_{phy})$ look like can be seen in figure \ref{fig:results_code_capcity_Z_ml_p_log_p_phy}.
...
18. auf den anhang mit den plots verweisen
}
...
The important properties of this approach are summarized in the following, for a more in-depth derivation, see \cite{pyfssa,mwpm_threshold_paper}.
...
These assumptions hold for $d\rightarrow \infty$ , for finite size small variation might appear \cite{pyfssa,mwpm_threshold_paper}.
...
Note that given our approach we do not directly take the data point with the highest and lowest index into account. 
...
An example is shown later in figure \ref{fig:results_code_capcity_Z_ml_p_log_p_phy_collapsed_data}.
