...
In this thesis, we investigate the threshold for Steane-type error correction circuits which performs syndrome extraction using logical encoded ancilla qubits in a fault-tolerant and single-shot setting.
...
which falls off to a threshold value of $2\%$ for repeated independent QEC cycles.
...
we are able to show an improvement of factor $3$, falling of to a factor of $2$ for the repeated QEC cycles. 
...
We perform those simulations using both a MWPM and a ML decoder to understand the influence of the decoder on the performance. 
We find that the ML decoder performs consistently better, even under restriction that we reduce the underlying error model to a phase- or bit-flip model. 

A central limitation of this work is the assumption of access to fault-tolerantly prepared logical ancilla states, which warrants further investigation.
